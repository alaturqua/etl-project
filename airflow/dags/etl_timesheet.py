...
def etl_timesheet():
    ...
    dbt_test = BashOperator(
        task_id= "dbt_test",
        bash_command="cd /git/sql-dbt && dbt ls && dbt test"
    )
    
    dbt_run = BashOperator(
        task_id= "dbt_run",
        bash_command="cd /git/sql-dbt && dbt ls && dbt seed && dbt run "
    )
    
    re_data_run = BashOperator(
        task_id= "re_data_run",
        bash_command="cd /git/sql-dbt && dbt run -m package:re_data"
    )
    
    re_data_overview_generate = BashOperator(
        task_id= "re_data_overview_generate",
        bash_command="cd /git/sql-dbt && re_data overview generate"
    )
    ...
    notify_data_anomalies = BashOperator(
        ...
        bash_command=f"""cd /git/sql-dbt && re_data notify slack \
